...
**Objectif :** Écrire un programme en Python qui simule la gestion de l'historique de navigation d'un navigateur web. L'utilisateur doit pouvoir visiter des pages web, revenir à la page précédente, aller à la page suivante (si un retour en arrière a été effectué précédemment), et afficher l'historique de navigation.
...
- Déclarez deux listes vides, `historique` et `pagesRetour`, pour représenter les stacks d'historique de navigation et de pages de retour en avant.

2. **Visiter une Page (`visiter`) :**

- Créez une fonction `visiter(url)` qui prend une URL en paramètre, l'ajoute à l'historique de navigation et efface toutes les URLs dans les pages de retour en avant, simulant ainsi la visite d'une nouvelle page.

3. **Retourner à la Page Précédente (`retour`) :**

- Implémentez une fonction `retour()` qui permet à l'utilisateur de revenir à la page précédente. Cette action déplace l'URL actuelle de l'historique vers les pages de retour en avant, simulant un retour en arrière dans la navigation.
